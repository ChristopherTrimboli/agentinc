    ...
    <div className={`group ${className}`}>
      ...
      <div className="relative">
        ...
        <input
          ...
          onChange={(e) =>
            onChange(
              type === "text" && inputClassName.includes("uppercase")
      ...
      <div
        ...
        style={{
          ...
          boxShadow: isSelected
            ? `0 0 30px ${agentColor}20, inset 0 1px 0 ${agentColor}20`
            : undefined,
        }}
      >
        ...
        {cardVariant === 2 && (
          <div
            ...
            style={{
              background: `linear-gradient(180deg, ${agentColor}, transparent 80%)`,
            }}
          />
        )}
        {cardVariant === 3 && (
    ...
    try {
      ...
      if (feeShareData.transactions?.length > 0) {
        ...
        for (let i = 0; i < feeShareData.transactions.length; i++) {
          ...
          if (lastError) {
            throw new Error(
              `Fee config transaction ${i + 1} failed after ${MAX_RETRIES + 1} attempts: ${lastError.message}`,
            );
          }
        }
      }
      ...
      if (feeShareData.bundles?.length > 0) {
        ...
        for (
          ...
          if (bundleLastError) {
            throw new Error(
              `Bundle ${bundleIdx + 1} failed after ${BUNDLE_MAX_RETRIES + 1} attempts: ${bundleLastError.message}`,
            );
          }
        }
      }
        ...
        <main className="h-[calc(100vh-72px)] mt-[72px] flex items-center justify-center px-4">
          <div className="text-center max-w-lg w-full">
            ...
            <p className="text-gray-400 mb-8 text-base">
              <span className="text-white font-semibold">{name}</span> is now
              live on Solana with{" "}
              ...
              <div className="w-full max-w-sm bg-gray-900/95 rounded-2xl p-8 border border-gray-800/60 shadow-2xl">
                <div className="text-center mb-8">
                  ...
                  <p className="text-gray-400 text-sm mt-1">
                    Please wait and approve any wallet prompts
                  </p>
                </div>
          ...
          <form onSubmit={handleLaunch} className="flex flex-col">
                  ...
                  {isLoadingAgents ? (
                    ...
                    <div className="grid gap-2 sm:gap-3 grid-cols-1 xs:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                      {agents.map((agent) => {
                        ...
                        const isDisabled =
                          !isSelected && selectedAgentIds.length >= 5;
            ...
            {currentStep === 1 && (
              <div className="flex flex-col">
                ...
                {/* Fee Structure */}
                <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-gray-800/50">
                  <label className="block text-[10px] sm:text-[11px] font-medium text-gray-500 mb-2 uppercase tracking-wider flex items-center gap-1">
                      ...
                      <button
                        ...
                        className={`text-left px-3 py-2.5 rounded-lg border transition-all duration-200 ${
                          feeStructure === key
                            ...
                            : "bg-gray-800/50 border-gray-700/40 hover:border-gray-600/40 hover:bg-gray-800/70"
                        }`}
                      >
                        <span
                          className={`text-xs font-semibold block ${feeStructure === key ? "text-purple-400" : "text-white/70"}`}
                        >
                          {config.label}
                        </span>
