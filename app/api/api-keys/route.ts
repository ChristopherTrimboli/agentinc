...
export async function POST(req: NextRequest) {
  ...
  const rateLimited = await rateLimitByUser(auth.userId, "api-keys-create", 10);
  ...
  let apiKey: { id: string; name: string; prefix: string; createdAt: Date };
  try {
    apiKey = await prisma.$transaction(async (tx) => {
      ...
      if (existingCount >= MAX_KEYS_PER_USER) {
        throw new Error(`Maximum of ${MAX_KEYS_PER_USER} active keys allowed`);
      }
