...
import { TWILIO_PRICING } from "@/lib/x402/config";
...
/**
 * Create Twilio tools with optional x402 billing.
 *
 * Each tool is wrapped with billedTool() which auto-charges the user
 * the real Twilio per-operation cost via x402 when the tool succeeds.
 * Costs are sourced from TWILIO_PRICING (lib/x402/config.ts).
 *
 * @param billingContext - Optional x402 billing context for usage charging
 */
export function createTwilioTools(billingContext?: BillingContext) {
  // ──────────────────────────────────────────────────────────────────────────
  // SMS
  // ──────────────────────────────────────────────────────────────────────────

  const sendSms = billedTool(
    "sendSms",
    {
  ...
  const playAudioCall = billedTool(
    "playAudioCall",
    {
      ...
      category: "Twilio",
      cost: TWILIO_PRICING.playAudioCall,
  ...
  const sendWhatsApp = billedTool(
    "sendWhatsApp",
    {
      ...
      category: "Twilio",
      cost: TWILIO_PRICING.sendWhatsApp,
  ...
  // ──────────────────────────────────────────────────────────────────────────
  // Status & History (free — no Twilio charge for reads)
  // ──────────────────────────────────────────────────────────────────────────

  const checkMessageStatus = billedTool(
    "checkMessageStatus",
    {
      description:
      ...
      category: "Twilio",
      cost: TWILIO_PRICING.checkMessageStatus,
  ...
  const getMessageHistory = billedTool(
    "getMessageHistory",
    {
      description:
      ...
      category: "Twilio",
      cost: TWILIO_PRICING.getMessageHistory,
  ...
  const checkTwilioConfig = billedTool(
    "checkTwilioConfig",
    {
      description:
      ...
      inputSchema: z.object({}),
      category: "Twilio",
      cost: TWILIO_PRICING.checkTwilioConfig,
      execute: async () => {
        const client = getTwilioClient();
...
/**
 * Default Twilio tools (no billing) for backward compatibility.
 * Use createTwilioTools(billingContext) for billed usage.
 */
export const twilioTools = createTwilioTools();
