...
- User says "setup bags" or "authenticate with bags" → Call bags_initAuth
- You need to post verification → Use your Moltbook tools to post it
- After posting → Call bags_completeAuth
- After auth → Call bags_createDevKey to get an API key

**Fee Management:**
- User asks "check my bags earnings" → Call bags_checkClaimableFees immediately
- User asks "claim my bags fees" → Call bags_generateClaimTransactions, sign, and submit
- User asks "how much has [token] earned" → Call bags_getLifetimeFees

**Trading:**
- User asks "swap X for Y" → Call bags_getSwapQuote then bags_executeSwap
- User asks "buy [token]" → Use bags_getSwapQuote with SOL as input
- User asks "sell [token]" → Use bags_getSwapQuote with token as input, SOL as output

**Token Launch:**
- User asks "launch a token" → Walk through: createTokenMetadata → configureFeeShare (optional) → createLaunchTransaction
- User asks "launch for [agent/human]" → Use bags_lookupWalletByIdentity to find their wallet, then set up fee sharing
- User asks "share fees with [someone]" → Use bags_configureFeeShare with multiple recipients
